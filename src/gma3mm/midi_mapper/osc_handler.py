...
class OSCHandler:
    ...
    def check_connection(self):
        ...
        def check_connection_helper():
            while not self._connected:
                self.log.debug(f"Checking OSC connection | IP: {self._client_ip} | Port: {self._client_port}")
    ...
    def send(self, address: str, message: str):
        ...
        if not self._connected:
            self.log.debug(f"OSC client not connected | Address: {address} | Message: {message}")
