...
from GMA3MM.enums import (
    MIDIMessageTypes,
    InboundNotes,
    OutboundNotes,
    OutboundControlSignals,
    KnobLEDState,
    InboundControlSignals,
    GMA3ExecMapToAPC40,
    APC40MapToGMA3Exec,
    ClipLaunchLEDState,
)
...
class State:
            ...
            with open(Path("update_blink.lua"), "r") as f:
                ...
                for b_exec in cls.blinkable:
                    OSC.send_message(
                        "/cmd",
                        "lua '"
                        + ";".join(lua).replace("[[]]", str(b_exec)).replace("\n", "")
                        + "'",
                    )
    ...
    def stop_blinking(cls, executor):
        ...
        if "primary_button" in exec_data:
            MIDI.output.send(
                mido.Message(
                    "note_on", note=exec_data["primary_button"], channel=0, velocity=1
                )
            )
        if "grid_button" in exec_data:
            MIDI.output.send(
                mido.Message(
                    "note_on",
                    note=exec_data["grid_button"],
                    channel=exec_data["channel"],
    ...
    for i in range(0, 8):
        MIDI.output.send(
            mido.Message(
                "note_on",
                channel=i,
                note=OutboundNotes.track_selection.value,
                velocity=0,
            )
        )
    MIDI.output.send(
        mido.Message("note_on", channel=0, note=OutboundNotes.master.value, velocity=0)
    )
...
def clear_apc40():
    for note in [
        OutboundNotes.clip_row_1.value,
        OutboundNotes.clip_row_2.value,
        OutboundNotes.clip_row_3.value,
        OutboundNotes.clip_row_4.value,
        OutboundNotes.clip_row_5.value,
        OutboundNotes.clip_stop.value,
        OutboundNotes.record_arm.value,
        OutboundNotes.solo.value,
        OutboundNotes.activator.value,
        OutboundNotes.clip_track.value,
        OutboundNotes.device_on_off.value,
        OutboundNotes.arrow_left.value,
        OutboundNotes.arrow_right.value,
        OutboundNotes.detail_view.value,
        OutboundNotes.rec_quant.value,
        OutboundNotes.midi_overdub.value,
        OutboundNotes.metronome.value,
    ]:
        for ch in range(0, 8):
            MIDI.output.send(mido.Message("note_on", note=note, channel=ch, velocity=0))
    for control in [
        OutboundControlSignals.track_level,
        OutboundControlSignals.master_level,
        OutboundControlSignals.device_knob_1,
        OutboundControlSignals.device_knob_2,
        OutboundControlSignals.device_knob_3,
        OutboundControlSignals.device_knob_4,
        OutboundControlSignals.device_knob_5,
        OutboundControlSignals.device_knob_6,
        OutboundControlSignals.device_knob_7,
        OutboundControlSignals.device_knob_8,
        OutboundControlSignals.track_knob_1,
        OutboundControlSignals.track_knob_2,
        OutboundControlSignals.track_knob_3,
        OutboundControlSignals.track_knob_4,
        OutboundControlSignals.track_knob_5,
        OutboundControlSignals.track_knob_6,
        OutboundControlSignals.track_knob_7,
        OutboundControlSignals.track_knob_8,
    ]:
        for ch in range(0, 8):
            MIDI.output.send(
                mido.Message(
                    "control_change", control=control.value, channel=ch, value=0
                )
            )
    ...
    for i in range(0, 8):
        MIDI.output.send(
            mido.Message(
                "control_change",
                control=OutboundControlSignals.device_knob_1_led.value + i,
    ...
    for i in range(0, 8):
        MIDI.output.send(
            mido.Message(
                "control_change",
                control=OutboundControlSignals.track_knob_1_led.value + i,
                value=KnobLEDState.off.value,
            )
        )
...
def fader_update(msg):
    ...
    if State.fader_values[ch] != -1:  # Value exists
        ...
        elif State.fader_values[ch] < State.fader_values[f"apcached{apc_ch}"]:
...
btn_signals = [
    InboundNotes.clip_row_1,
    InboundNotes.clip_row_2,
    InboundNotes.clip_row_3,
    InboundNotes.clip_row_4,
    InboundNotes.clip_row_5,
    InboundNotes.clip_stop,
    InboundNotes.record_arm,
    InboundNotes.solo,
    InboundNotes.activator,
    InboundNotes.clip_track,
    InboundNotes.device_on_off,
    InboundNotes.arrow_left,
    InboundNotes.arrow_right,
    InboundNotes.detail_view,
    InboundNotes.rec_quant,
    InboundNotes.midi_overdub,
    InboundNotes.metronome,
    InboundNotes.stop_all_clips,
]
...
@route_midi(
    ...
    1,
    [
        InboundControlSignals.device_knob_1,
        InboundControlSignals.device_knob_2,
        InboundControlSignals.device_knob_3,
        InboundControlSignals.device_knob_4,
        InboundControlSignals.device_knob_5,
        InboundControlSignals.device_knob_6,
        InboundControlSignals.device_knob_7,
        InboundControlSignals.device_knob_8,
        InboundControlSignals.track_knob_1,
        InboundControlSignals.track_knob_2,
        InboundControlSignals.track_knob_3,
        InboundControlSignals.track_knob_4,
        InboundControlSignals.track_knob_5,
        InboundControlSignals.track_knob_6,
        InboundControlSignals.track_knob_7,
        InboundControlSignals.track_knob_8,
    ],
)
def knob(msg):
...
def exec_data(address, *args):
    ...
    if int(index) in State.fader_values:
        ...
        State.fader_values[f"apcached{apc_index}"] = State.fader_values[
            f"apc{apc_index}"
        ]
    ...
    if "fader" in exec_map_data:
        MIDI.output.send(
            mido.Message(
                "control_change",
                control=exec_map_data["fader"],
                channel=exec_map_data["channel"],
                value=fader_value,
            )
        )
        if "primary_button" in exec_map_data:
            MIDI.output.send(
                mido.Message(
                    "note_on",
                    note=exec_map_data["primary_button"],
                    channel=exec_map_data["channel"],
                    velocity=1,
                )
            )
    if "grid_button" in exec_map_data:
        if index in State.blinkable:
            MIDI.output.send(
                mido.Message(
                    "note_on",
                    note=exec_map_data["grid_button"],
                    channel=exec_map_data["channel"],
                    velocity=ClipLaunchLEDState.yellow.value,
                )
            )
            ...
            MIDI.output.send(
                mido.Message(
                    ...
                    note=exec_map_data["grid_button"],
                    channel=exec_map_data["channel"],
                    velocity=ClipLaunchLEDState.green.value,
                )
            )
        if "primary_button" in exec_map_data:
            MIDI.output.send(
    ...
    if "knob" in exec_map_data:
        if fader_type:
            MIDI.output.send(
                mido.Message(
                    "control_change",
                    control=exec_map_data["knob"] + 8,
                    value=KnobLEDState.volume.value,
                )
            )
            MIDI.output.send(
                mido.Message(
                    "control_change", control=exec_map_data["knob"], value=fader_value
                )
            )
        else:
            MIDI.output.send(
            ...
            MIDI.output.send(
                mido.Message("control_change", control=exec_map_data["knob"], value=64)
            )
            ...
            if "grid_button" in exec_data:
                MIDI.output.send(
                    mido.Message(
                        ...
                        note=exec_data["grid_button"],
                        channel=exec_data["channel"],
                        velocity=ClipLaunchLEDState.off.value,
                    )
                )
        sleep(0.5)
...
MIDI.output.send(
    mido.Message(
        "note_on", channel=0, note=OutboundNotes.track_selection.value, velocity=1
    )
)
...
MIDI.output.send(
    mido.Message.from_bytes(b"\xF0\x47\x00\x73\x60\x00\x04\x42\x01\x01\x01\xF7")
)  # Set APC40 to mode 3
